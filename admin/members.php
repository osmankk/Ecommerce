                        ...
                        <div class='container'>
                                    ...
                                    foreach ($rows as $row) {
                                        ...
                                        echo "<td>".
                                              ...
                                              '<a href="members.php?do=Delete&user_id='.$row['UserID'].'" class="comfirm btn btn-danger"><i class="glyphicon glyphicon-remove"></i>Delete</a>';
                                               if ($row['RegStatus']== 0) {
                                                echo '<a href="members.php?do=Activate&user_id='  .$row['UserID'].'" class="btn btn-info"><i class="glyphicon glyphicon-ok"></i>Activate</a>';
                           ...
                           <h1 class='text-center'>Add New Member</h1>
                            ...
                            elseif($do == 'Insert') {
                                ...
                                if ( $check == 1) {
                                        $var = " USER NAME ALREDY <strong>EXISTED !!!!</strong>";
                                        $error_form[] = $var ;//div dont work
                                        DirectionHome( "<div class='alert alert-danger'>".$var."</div>",'back',3);
                                } 
...
<?php }
            ...
            elseif($do == 'Update') {
                ...
                $user  = isset($_GET['user_id'])&& is_numeric($_GET['user_id'])?intval(($_GET['user_id'])):0;
                    ...
                    $stmt->execute(array($user ));
                    if ($stmt->rowCount() > 0) {
                    $Msg = "<div class='alert alert-success'>".$stmt->rowCount()."member has been Activated</div>";
                    DirectionHome($Msg,'members.php',);
                    }else {
